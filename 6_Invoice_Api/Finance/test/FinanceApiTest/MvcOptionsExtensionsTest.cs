...
namespace FinanceApiTest
{
    ...
    public sealed class MvcOptionsExtensionsTest
    {
        ...
        [Fact]
        public void SetOutputMediaType_should_set_mediaType()
        {
            // Arrange
            var expected = new MediaTypeHeaderValue("application/vnd.test+json");

            // Act
            testSubject.SetOutputMediaType(expected);

            var actual =
                testSubject.OutputFormatters.OfType<JsonOutputFormatter>().First().SupportedMediaTypes.First();
